...
export function NewsGrid({
  ...
  return (
    <div className={`news-grid${layout === "uniform" ? " news-grid-uniform" : ""}`}>
      {items.map((item, index) => (
        <a className={`news-card ${layout === "featured" && index === 0 ? "news-featured" : ""}`} href={item.link} target="_blank" rel="noreferrer" key={item.id}>
          ...
          <h3>{item.title}</h3>
          {layout === "featured" && index === 0 && item.summary && <p>{item.summary}</p>}
