...
- D1: `soccer-korea-db`
- 현재 무료 자동 수집: OpenLigaDB 월드컵 경기·득점 이벤트, BBC Football RSS, Transfermarkt 공식 RSS
- `football-data.org`와 API-Football adapter는 구현되어 있으나 실제 무료 API 키가 없어 운영 환경에서는 비활성화
...
- 경기 일정·스코어, 리그 순위, 가십, 확정 이적, 몸값 영역
...
- `ENABLE_FOOTBALL_DATA=true`일 때 football-data.org 경기·순위 adapter 사용 가능
- `ENABLE_API_FOOTBALL=true`일 때 API-Football 확정 이적 및 경기별 팀·선수 상세 통계 adapter 사용 가능
- 키 기반 공급자는 현재 자동 Cron에서 제외하며 관리자 수동 동기화로 호출량을 통제
...
- 2026-07-06 재동기화 기준 OpenLigaDB 월드컵 경기 100건, 득점 이벤트 288건 D1 저장 확인
